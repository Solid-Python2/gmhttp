...
const (
	...
	minVersion = VersionGMTLS // change
	maxVersion = VersionGMTLS // change
)
...
type Config struct {
	...
	//国密是双证书体系-服务端启动需要三个证书SS SE CA证书
	GetCertificateGM func(*ClientHelloInfo) ([]*Certificate, error)
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		GetCertificateGM:            c.GetCertificateGM,
...
*/
func (c *Config) getCertificateGM(clientHello *ClientHelloInfo) ([]*Certificate, error) {

	if c.GetCertificateGM != nil &&
		(len(c.Certificates) == 0 || len(clientHello.ServerName) > 0) {
		certs, err := c.GetCertificateGM(clientHello)
	...
	//这个地方处理一下 可以认为至少需要三个证书
	if len(c.Certificates) < 3 {

		return nil, errors.New("tls: GM no certificates configured")
	}
...
/*
func initDefaultCipherSuites_old() {
...
func initDefaultCipherSuites() {
	...
	var topCipherSuites []uint16
	topCipherSuites = []uint16{
		TLS_ECC_SM2_WITH_SM4_256_SM3,
	}

	//varDefaultCipherSuites = make([]uint16, 0, len(cipherSuites))
	varDefaultCipherSuites = make([]uint16, 0, len(topCipherSuites))
