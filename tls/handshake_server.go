...
type serverHandshakeState struct {
	...
	sm2Ok bool
	// sm2SignOk             bool //国密处理
	sm2DecryptOk          bool //国密处理
	sessionState          *sessionState
	finishedHash          finishedHash
	masterSecret          []byte
	certsFromClient       [][]byte
	cert                  *Certificate   //SS证书
	encert                *Certificate   //SE加密证书
	certs                 []*Certificate //SS CA SE 证书
...
Curves:
	...
	//国密多证书
	hs.certs, err = c.config.getCertificateGM(hs.clientHelloInfo())
	if err != nil {
		c.sendAlert(alertInternalError)
		return false, err
	}
	...
	hs.cert = hs.certs[0]
	hs.encert = hs.certs[len(hs.certs)-1]
	// //hs.cert, err = c.config.GetCertificateGM(hs.clientHelloInfo())
	// if err != nil {
	// 	c.sendAlert(alertInternalError)
	// 	return false, err
	// }
	...
	// 添加加密证书
	if priv, ok := hs.encert.PrivateKey.(crypto.Decrypter); ok {
		switch priv.Public().(type) {
		case *sm2.PublicKey:

			hs.sm2DecryptOk = true
		default:
			c.sendAlert(alertInternalError)
			return false, fmt.Errorf("tls: unsupported decryption encert key type (%T)", priv.Public())
		}
	}
...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	//处理国密的3个证书 0430
	certMsg.certificates = make([][]byte, 3)
	for i := 0; i < len(hs.certs); i++ {
		certMsg.certificates[i] = append(certMsg.certificates[i], hs.certs[i].Certificate[0]...)
	}
	...
	//国密三个证书
	skx, err := keyAgreement.generateServerKeyExchange(c.config, hs.certs, hs.clientHello, hs.hello)
	...
	preMasterSecret, err := keyAgreement.processClientKeyExchange(c.config, hs.certs, ckx, c.vers)
	if err != nil {
		c.sendAlert(alertHandshakeFailure)
		return err
	}
...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	...
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if candidate.flags != suiteGMTLS11 {
				continue
			}
			//
			if version != VersionGMTLS && version < VersionTLS12 && candidate.flags&suiteTLS12 != 0 {
				//
				continue
			}
			//
