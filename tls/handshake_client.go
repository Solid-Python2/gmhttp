...
NextCipherSuite:
	...
	_, err := io.ReadFull(c.config.rand(), hello.random) //这个没有用时间，直接就是随机数
	if err != nil {
	...
	//hellohex := hello.marshal()
	hellohex := hello.marshalGM() //国密版本有些调整，高版本会有很多扩容的信息，国密的版本目前暂时不需要

	if _, err := c.writeRecord(recordTypeHandshake, hellohex); err != nil {
		return err
	}
	...
	//  add 国密版本修改
	if !ok || vers != VersionGMTLS {

		// 	// TLS 1.0 is the minimum version supported as a client.
		c.sendAlert(alertProtocolVersion)
		return fmt.Errorf("tls: gmtls server selected unsupported protocol version %x", serverHello.vers)
	}
	c.vers = vers
	c.haveVers = true
	...
	if isResume || (len(c.config.Certificates) == 0 && c.config.GetClientCertificate == nil) {
		hs.finishedHash.discardHandshakeBuffer() //如果是半握手协议，则需要丢弃掉原来的数据
	}

	//hs.finishedHash.Write(hs.hello.marshal())
	hs.finishedHash.Write(hs.hello.marshalGM())
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		case *sm2.PublicKey: //国密只使用sm2 不兼容其他证书
			break
		default:
	...
	//fmt.Println("ServerHello ocspStapling->", hs.serverHello.ocspStapling)
