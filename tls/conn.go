...
func (hc *halfConn) decrypt(b *block) (ok bool, prefixLen int, alertValue alert) {
	...
	if hc.mac != nil {
		...
		n := len(payload) - macSize - paddingLen                //需要减去
		n = subtle.ConstantTimeSelect(int(uint32(n)>>31), 0, n) // if n < 0 { n = 0 }
		lenn = n - 16

		b.data[3] = byte(lenn >> 8)
		b.data[4] = byte(lenn)
		remoteMAC := payload[n : n+macSize]

		localMAC := hc.mac.MAC(hc.inDigestBuf, hc.seq[0:], b.data[:recordHeaderLen], payload[16:lenn+16], payload[n+macSize:])
		...
		b.resize(recordHeaderLen + lenn)
		copy(b.data[recordHeaderLen:], payload[16:lenn+16])

	}
	hc.incSeq()
	return true, recordHeaderLen, 0
...
func (hc *halfConn) encrypt(b *block, explicitIVLen int) (bool, alert) {
	...
	if hc.cipher != nil {
		...
		case cbcMode:
			...
			if explicitIVLen > 0 {
				c.SetIV(payload[:explicitIVLen])  //这里设置随机种子
				payload = payload[explicitIVLen:] //这里加密数据还有hmac的值
			}
...
func (c *Conn) writeRecordLocked(typ recordType, data []byte) (int, error) {
	...
	for len(data) > 0 {
		...
		var cbc cbcMode //add c.out.version == VersionGMTLS
		if c.out.version >= VersionTLS11 || c.out.version == VersionGMTLS {
			var ok bool
		...
		b.data[0] = byte(typ) //握手协议类型
		vers := c.vers
		if vers == 0 {
			// Some TLS servers fail if the record version is
			// greater than TLS 1.0 for the initial ClientHello.
			//vers = VersionTLS10
			vers = VersionGMTLS //使用国密的版本
		}
		b.data[1] = byte(vers >> 8)
		b.data[2] = byte(vers)   //版本信息
		b.data[3] = byte(m >> 8) //长度信息
		b.data[4] = byte(m)
		if explicitIVLen > 0 {
		...
		c.out.encrypt(b, explicitIVLen) //这里对数据进行了加密 加密之后TASSL 能正常的解析出来数据
...
func (c *Conn) Handshake() error {
	...
	// 上面考虑的是一些并发的情况，可能在读写数据的时候链路被挂起，
	// 所以握手的相关需要加锁确认是否已经完成握手
